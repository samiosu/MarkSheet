...
test('正答の出力再読込は配点を保持し、無効な配点を拒否する', async ({ page }) => {
  ...
  const answerKeyRecords = samplePair().answerKey.questions.map((question) => ({ label: question.label, answer: question.answer, points: 'points' in question ? question.points : 1 }))
  expect(json.document).toEqual(answerKeyRecords)
  expect(JSON.parse(json.text).every((q: object) => Object.keys(q).sort().join(',') === 'answer,label,points')).toBe(true)
  expect(csv.text.split('\r\n')[0]).toBe('\uFEFFlabel,answer,points')
...
test('外部の2列CSVは番号の先頭ゼロを保ち、欠落・重複・余分な項目を拒否する', async ({ page }) => {
  ...
  await expect(page.getByRole('alert')).toContainText('配点付きファイルは正答の読込先で使用してください')
  await expect(row(page, '001').getByRole('radio', { name: '1', exact: true })).toBeChecked()
})
